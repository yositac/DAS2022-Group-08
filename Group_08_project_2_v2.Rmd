...
The data after we remove the missing values, 858 observations were obtained. And data set includes 8 variables in total. To answer the research question, the Quality class serves as response variables...

(outcome is good or poor, suit for binary logistic regression in GLM, brief describe the reason we choose GLM's logistic family)
```{r na value removal}
...
Receiver Operating Characteristic curve(ROC curve) is a method used to indicates the determination ability. It is uaually apply to the binary system, which suit for the final model. 
